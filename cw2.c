...
{
    unsigned short signature;    // îïðåäåëåíèå òèïà ôàéëà
    unsigned int filesize;       // ðàçìåð ôàéëà
    unsigned short reserved1;    // äîëæåí áûòü 0
    unsigned short reserved2;    // äîëæåí áûòü 0
    unsigned int pixelArrOffset; // íà÷àëüíûé àäðåñ áàéòà, â êîòîðîì íàõîäÿòñÿ äàííûå èçîáðàæåíèÿ (ìàññèâ ïèêñåëåé)
} BitmapFileHeader;

typedef struct BitmapInfoHeader
{
    unsigned int headerSize;          // ðàçìåð ýòîãî çàãîëîâêà â áàéòàõ
    unsigned int width;               // øèðèíà èçîáðàæåíèÿ â ïèêñåëÿõ
    unsigned int height;              // âûñîòà èçîáðàæåíèÿ â ïèêñåëÿõ
    unsigned short planes;            // êîë-âî öâåòîâûõ ïëîñêîñòåé (äîëæíî áûòü 1)
    unsigned short bitsPerPixel;      // ãëóáèíà öâåòà èçîáðàæåíèÿ
    unsigned int compression;         // òèï ñæàòèÿ; åñëè ñæàòèÿ íå èñïîëüçóåòñÿ, òî çäåñü äîëæåí áûòü 0
    unsigned int imageSize;           // ðàçìåð èçîáðàæåíèÿ
    unsigned int xPixelsPerMeter;     // ãîðèçîíòàëüíîå ðàçðåøåíèå (ïèêñåëü íà ìåòð)
    unsigned int yPixelsPerMeter;     // âåðòèêàëüíîå ðàçðåøåíèå (ïèêñåëü íà ìåòð)
    unsigned int colorsInColorTable;  // êîë-âî öâåòîâ â öâåòîâîé ïàëèòðå
    unsigned int importantColorCount; // êîë-âî âàæíûõ öâåòîâ (èëè 0, åñëè êàæäûé öâåò âàæåí)
} BitmapInfoHeader;
...
void check_data_for_function_split(Rgb** arr, unsigned int H, unsigned int W,  int count_y,  int count_x, int thicknessm, char* color){
	int* color_check;
	if(color != NULL){
		color_check = parsing_args(color);
	}
	else{
		printf("uncorrect data\n");
		exit(ERROR_UNCORRECT_ARGUMENTS);
